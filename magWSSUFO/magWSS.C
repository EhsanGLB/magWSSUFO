...
namespace Foam
{
    defineTypeNameAndDebug(magWSS, 0);
}


// * * * * * * * * * * * * * Protected Member Functions  * * * * * * * * * * //

void Foam::magWSS::writeFileHeader(const label i)
{
...
)
{
    forAllConstIter(labelHashSet, patchSet_, iter)
    {
        ...
        scalarField& ssp = WSS.boundaryField()[patchI];
    ...
    phiName_("phi")
{
    ...
    if (active_)
    {
        ...
        mesh.objectRegistry::store(magWSSPtr);
    }
...
void Foam::magWSS::read(const dictionary& dict)
{
    if (active_)
    {
        ...
        else
        {
            ...
            forAllConstIter(labelHashSet, patchSet_, iter)
            {
                ...
                {
                    WarningIn("void magWSS::read(const dictionary&)")
...
void Foam::magWSS::execute()
{
    ...
    if (active_)
    {
        ...
        {
            FatalErrorIn("void Foam::magWSS::execute()")
        ...
        calcWSS(mesh, Reff(), magWSS);
    }
}


void Foam::magWSS::end()
{
    if (active_)
    {
        execute();
...
void Foam::magWSS::timeSet()
{
    // Do nothing
}


void Foam::magWSS::write()
{
    if (active_)
    {
        functionObjectFile::write();

        const volScalarField& magWSS =
            obr_.lookupObject<volScalarField>(type());

        if (log_) Info<< type() << " " << name_ << " output:" << nl
            << "    writing field " << magWSS.name() << nl
            << endl;
